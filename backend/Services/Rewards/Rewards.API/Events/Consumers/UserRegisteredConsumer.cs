...
public class UserRegisteredConsumer : BackgroundService
{
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        // Use a unique queue name so Rewards gets its own copy of the event
        // independent of the Wallet service's consumer
        var queueName = $"{RabbitMQQueues.UserRegistered}.rewards";
        ...
        _logger.LogInformation("Rewards UserRegisteredConsumer started");
    ...
    private async Task OnMessageReceived(object sender, BasicDeliverEventArgs args)
    {
        ...
        try
        {
            ...
            await rewardsService.CreateAccountAsync(@event.UserId);

            _channel!.BasicAck(args.DeliveryTag, false);
            _logger.LogInformation("Loyalty account created for user {UserId}", @event.UserId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error in Rewards UserRegisteredConsumer");
